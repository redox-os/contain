    ...
    match sys_users.get_by_name(&username) {
        Some(user) => {
            if user.verify_passwd(&pass) {
                ...
                Some((command, user.uid, user.home.to_owned()))
            } else {
                None
            }
        }
...
fn login_window(
    launcher_cmd: &str,
    launcher_args: &[String],
) -> Result<Option<(Command, usize, String)>, String> {
                        ...
                        match key_event.scancode {
                            ...
                            orbclient::K_ENTER => {
                                if item == 0 {
                                    ...
                                    if let Some(command_tuple) = login_command(
                                        &username,
                                        &password,
                                        launcher_cmd,
                                        launcher_args,
                                    ) {
                                        return Ok(Some(command_tuple));
                                    } else {
                                        item = 0;
                                        password.clear();
                                        failure = true
...
fn main() -> io::Result<()> {
    ...
    let _ = RedoxLogger::new()
        ...
        .with_process_name("contain_orblogin".into())
        .enable();
